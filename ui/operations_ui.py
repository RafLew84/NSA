# -*- coding: utf-8 -*-
"""
This module creates the operations UI section with dropdowns, sliders, and buttons for 
selecting and applying image operations.
...
def create_operations_ui(
    ...
    """
    Create the operations UI section with dropdowns, sliders, and buttons for selecting and applying image operations.

    Args:
        root (tk.Tk): The root Tkinter window.
        selected_operation (tk.StringVar): A Tkinter StringVar that stores the currently selected operation.

    Returns:
        tuple: A tuple containing the operations UI section frame and the selected operation.
    """
    ...
    def choose_process_options_dropdownOnChange(selected_option):
        """
        Handle the selection change event for the process dropdown.
        Clear previous UI elements and create new ones based on the selected option.
        """
        try:
            ...
            logger.error(f"Error processing dropdown selection: {e}")


    def update_sliders_onChange(event=None):
        """
        Handle the slider change event to update the displayed image.
        """
        try:
            process_and_display_image()
        except Exception as e:
            logger.error(f"Error updating sliders: {e}")

    def process_and_display_image():
        """
        Process the image using the selected operation and display the result.
        """
        try:
            params = {}
    ...
    def apply_preprocessing_onClick():
        """
        Apply the selected preprocessing operation to the image.
        """
        try:
            params = {}
            img = selected_item_manager.selected_item.image_for_processing
    ...
    def apply_processing_operation(params, img):
        """
        Apply the selected processing operation to the image based on the given parameters.
        ...
        Returns:
            tuple: The processed image and the name of the operation.
        """
        try:
            ...
            logger.error(f"Error applying processing operation: {e}")
            raise
    
    def get_values_from_preprocess_menu_items(params):
        """
        Retrieve the values from the preprocess menu items and update the parameters dictionary.

        Args:
            params (dict): The dictionary to store the parameters.
        """
        try:
